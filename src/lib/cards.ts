  ...
  for (let i = 0; i < mappings.suits.length; i++) {
    if (mappings.suits[i].name.toLowerCase() == suit.toLowerCase()) {
      for (let j = 0; j < mappings.suits[i].cards.length; j++) {
        if (
          mappings.suits[i].cards[j].value.toLowerCase() == card.toLowerCase()
        ) {
          var cardname =
            mappings.suits[i].name.toLowerCase() +
            " " +
...
export function GetCardImageUrl(
  ...
  if (!suit || !card) return "/cards/all/1.png";
  ...
  else cval = parseInt(c);

  if (c == "A" && s == "JO") id = 79;
  else if (c == "B" && s == "JO") id = 80;
  else id = sval * 13 + cval - 1;
...
function convertSuitAndCardToId(suit: string, card: string) {
  ...
  if (suit == "cryptography") firstPart = "CR";

  if (suit == "data-validation-&-encoding") firstPart = "DV";

  if (suit == "authentication") firstPart = "AC";

  if (suit == "authorization") firstPart = "AZ";

  if (suit == "session-management") firstPart = "SM";

  if (suit == "cornucopia") firstPart = "CO";
